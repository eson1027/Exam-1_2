...
public:
  ...
  sc_core::sc_time delay(transaction_type &trans) {//傳一筆資料會有的delay
    // Note that 4 means bus width is 4 bytes; not good enough coding.
    return (1 + trans.get_data_length() / 4) * clock_period; //model interconnect delay,至少1ns
  ...
  void initiatorBTransport(int SocketId, transaction_type &trans,
    ...
    // It is a static port ?
    initiator_socket_type *decodeSocket = &i_skt[portId];//bus 本身的initiator socket
    if (m_is_address_masked) {
      trans.set_address(offset);
    }
    t = t + delay(trans); //add interconnect delay(累加transaction的delay)
